...
def audit_booking_flow(page: Page, url: str, name: str, width: int, height: int) -> list[str]:
    ...
    page.wait_for_timeout(650)
    check(page.locator(".table-focus").is_visible(), "phone-flow: table focus is not visible", failures)
    page.locator(".view-from-seat").tap()
    page.wait_for_timeout(650)
    check(page.locator(".spatial-preview.seat-view").is_visible(), "phone-flow: seat view is not visible", failures)
