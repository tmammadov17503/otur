...
def audit_booking_flow(page: Page, url: str, name: str, width: int, height: int) -> list[str]:
    ...
    check(page.locator(".zoom-controls").count() == 0, "phone-flow: zoom controls should be removed", failures)
    plan_metrics = page.locator(".plan-viewport").evaluate("el => ({scrollWidth: el.scrollWidth, clientWidth: el.clientWidth, scrollHeight: el.scrollHeight, clientHeight: el.clientHeight})")
    check(plan_metrics["scrollWidth"] <= plan_metrics["clientWidth"] + 1 and plan_metrics["scrollHeight"] <= plan_metrics["clientHeight"] + 1,
          f"phone-flow: floor plan should fit without internal scrolling ({plan_metrics})", failures)
    focus_box = table_focus.bounding_box()
    check(focus_box is not None and focus_box["y"] >= -1 and focus_box["y"] + focus_box["height"] <= height + 1,
          f"phone-flow: selected-table view should fit in the viewport ({focus_box})", failures)

    page.locator(".reserve-selected-table").tap()
    check(page.locator(".demo-account-button").count() == 0, "phone-flow: demo login must not be shown", failures)
    page.locator(".account-tabs button").nth(1).tap()
    ...
    page.locator(".account-form .confirm-button").tap()
